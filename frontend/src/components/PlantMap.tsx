...
function zoneStyle(zone: ZoneState) {
  ...
  const opacity = zone.crs > 0 ? Math.min(0.75, 0.2 + zone.crs / 120) : 0.2;
...
export function PlantMap({
  ...
  return (
    <MapContainer
      ...
      style={{ minHeight: 420, background: '#0B0F14' }}
      scrollWheelZoom
    >
      ...
      {evacRoutes.map((route, idx) =>
        route.route.length >= 2 ? (
          <Polyline
            ...
            pathOptions={{ color: '#FF3B30', weight: 3, dashArray: '8 6' }}
          />
        ) : null,
      )}
      ...
      {workers.map((w) => (
        <CircleMarker
          ...
          center={[w.lat, w.lng]}
          radius={6}
          pathOptions={{ color: '#F2F4F7', weight: 1, fillColor: '#0A84FF', fillOpacity: 1 }}
        >
