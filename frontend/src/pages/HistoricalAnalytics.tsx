...
export function HistoricalAnalyticsPage() {
  ...
  if (!data) return <div className="p-4 text-text-secondary">Loading analytics...</div>;
    ...
    <div className="p-4 h-full flex flex-col">
      <header className="mb-4 flex items-center justify-between">
        ...
        <button
          ...
          className="flex items-center gap-1 text-xs bg-accent text-white px-3 py-1.5 rounded"
        >
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        {[
          ...
          <div key={m.label} className="bg-bg-surface border border-gray-800 rounded-lg p-3">
      ...
      <div className="grid grid-cols-12 gap-4 flex-1 min-h-0">
        <div className="col-span-7 bg-bg-surface border border-gray-800 rounded-lg p-4 overflow-y-auto">
          <h3 className="text-xs font-medium text-text-secondary uppercase mb-3 flex items-center gap-1">
          ...
          <table className="w-full text-xs">
            ...
            <tbody>
              {data.motif_performance.map((m) => (
                <tr key={m.motif_id} className="border-b border-gray-800/50">
        ...
        <div className="col-span-5 bg-bg-surface border border-gray-800 rounded-lg p-4 overflow-y-auto">
          <h3 className="text-xs font-medium text-text-secondary uppercase mb-3">CRS Timeline (Replay)</h3>
          <p className="text-[10px] text-text-secondary mb-2">Offset: T{data.replay_offset}m · Active risks: {data.active_risks}</p>
          {data.crs_timeline.map((t) => (
            <div key={t.offset} className="flex items-center gap-2 mb-2 text-xs">
              <span className="font-mono text-text-secondary w-12">T{t.offset}m</span>
