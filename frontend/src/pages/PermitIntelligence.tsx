  ...
  return (
    <div className="p-4 h-full flex flex-col">
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="bg-bg-surface border border-line rounded-xl shadow-card p-3">
          <div className="text-2xl font-bold tabular-nums">{permits.length}</div>
          <div className="text-xs text-ink-secondary">Active / Scheduled</div>
        </div>
        <div className="bg-sev-critical/10 border border-sev-critical/30 rounded-lg p-3">
          ...
          <div className="text-xs text-ink-secondary">Live Conflicts</div>
        </div>
        <div className="bg-bg-surface border border-line rounded-xl shadow-card p-3">
          <div className="text-2xl font-bold tabular-nums">{permits.filter((p) => p.status === 'ACTIVE').length}</div>
          <div className="text-xs text-ink-secondary">Active Permits</div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-2">
        ...
        {permits.map((p) => (
          <div key={`${p.permit_id}-${p.status}`} className="bg-bg-surface border border-line rounded-xl shadow-card p-4">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="text-[10px] text-ink-secondary">{p.status}</span>
                </div>
                <div className="text-xs text-ink-secondary">{p.zone_id} · {p.zone_name} · {p.type}</div>
              ...
              {p.conflicts.length > 0 && (
                <button
                  ...
                  className="text-xs bg-bg-base border border-line px-3 py-1.5 rounded hover:border-sev-active shrink-0"
                >
                  Override (audited)
                </button>
              )}
            ...
            {overrideId === p.permit_id && (
              <div className="mt-3 pt-3 border-t border-line">
                <label className="text-xs text-ink-secondary block mb-1">Justification required (§9.3)</label>
                <textarea
                  value={justification}
                  onChange={(e) => setJustification(e.target.value)}
                  className="w-full bg-bg-base border border-line rounded p-2 text-sm h-16"
                ...
                <div className="flex gap-2 mt-2">
                  <button
                    ...
                    className="text-xs bg-accent-peach text-ink-primary px-3 py-1.5 rounded disabled:opacity-50 flex items-center gap-1"
                  >
                    <Shield size={12} /> Submit Override
                  </button>
                  <button onClick={() => setOverrideId(null)} className="text-xs text-ink-secondary px-3 py-1.5">
                    Cancel
                  </button>
                </div>
              </div>
