...
import { ArrowLeft, Shield, AlertOctagon, CheckCircle, XCircle } from 'lucide-react';
import { api } from '../api';
...
export function RiskDetail() {
  ...
  const handleAck = async () => {
    ...
    try {
      ...
      const updated = await api.getRisk(id);
      setRisk(updated);
    } finally {
      setActing(false);
    }
  ...
  const handleEmergency = async () => {
    if (!risk) return;
    if (!confirm('Declare emergency for zone ' + risk.zone_id + '?')) return;
    setEmergency(true);
    ...
    <div className="p-4 h-full flex flex-col">
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={handleEmergency} className="flex items-center gap-1 bg-sev-critical/20 text-sev-critical border border-sev-critical/40 text-xs px-3 py-1.5 rounded">
            <AlertOctagon size={14} /> Declare Emergency
          </button>
        </div>
      </div>

      {emergency && (
        <div className="bg-sev-critical/10 border border-sev-critical/40 rounded p-3 mb-4 text-sm">
          Emergency declared for {risk.zone_id}. Evacuation initiated · 3 workers notified · Evidence locked.
        </div>
      )}
      ...
      <div className="flex-1 grid grid-cols-12 gap-4 min-h-0 overflow-hidden">
        ...
        <div className="col-span-5 space-y-4 overflow-y-auto">
          {evidence && (
            <div className="bg-bg-surface rounded-lg border border-gray-800 p-4">
              <h3 className="text-xs font-medium text-text-secondary uppercase mb-2">Multi-Agent Analysis</h3>
              {Object.entries(evidence.agent_findings).map(([agent, finding]) => (
                <div key={agent} className="mb-2">
                  <div className="text-xs font-medium text-accent capitalize">{agent} Agent</div>
                  <p className="text-xs text-text-secondary">{finding}</p>
                </div>
              ))}
            </div>
          )}
