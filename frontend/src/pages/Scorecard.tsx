...
export function ScorecardPage() {
  ...
  if (!scorecard) return <div className="p-4 text-text-secondary">Loading scorecard...</div>;
    ...
    <div className="p-4">
      <header className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-text-secondary">§9a.5 — PRAHARI vs Single-Sensor Baseline on Coke-Oven Scenario</p>
        </div>
        <div className="flex gap-2">
          <button onClick={runReplay} disabled={replaying} className="flex items-center gap-1.5 bg-accent text-white text-xs px-3 py-1.5 rounded">
            <Play size={14} /> {replaying ? `Replaying step ${replayStep + 1}/5...` : 'Run Live Replay'}
          </button>
          <button onClick={async () => { await api.resetDemo(); load(); }} className="flex items-center gap-1.5 bg-bg-elevated border border-gray-700 text-xs px-3 py-1.5 rounded">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-bg-surface rounded-lg border border-gray-800 p-4">
          <div className="text-xs text-text-secondary uppercase mb-2">Single-Sensor Baseline (SCADA)</div>
        ...
        <div className="bg-bg-surface rounded-lg border border-sev-ok/30 p-4">
          <div className="text-xs text-text-secondary uppercase mb-2">PRAHARI (Compound Detection)</div>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        {[
          ...
          <div key={m.label} className="bg-bg-surface rounded-lg border border-gray-800 p-3 text-center">
            <div className="text-[10px] text-text-secondary uppercase">{m.label}</div>
            <div className="text-2xl font-bold tabular-nums mt-1">{m.value}</div>
            <div className="text-[10px] text-text-secondary">{m.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div className="bg-bg-surface rounded-lg border border-gray-800 p-4">
        ...
        <div className="space-y-2">
          {scorecard.timeline.map((t) => (
            <div key={t.offset} className="grid grid-cols-12 gap-2 text-xs items-center py-2 border-b border-gray-800/50">
              <div className="col-span-1 font-mono text-text-secondary">T{t.offset >= 0 ? '+' : ''}{t.offset}m</div>
              <div className="col-span-5 text-text-secondary">{t.description}</div>
              <div className={`col-span-3 font-medium ${t.baseline === 'SILENT' ? 'text-text-secondary' : 'text-sev-critical'}`}>
                Baseline: {t.baseline}
              </div>
              <div className={`col-span-3 font-medium ${t.prahari === 'SILENT' ? 'text-text-secondary' : 'text-sev-ok'}`}>
                PRAHARI: {t.prahari}
              </div>
            </div>
          ))}
